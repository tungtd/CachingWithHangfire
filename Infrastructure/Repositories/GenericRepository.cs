...
namespace Infrastructure.Repositories
{
    public class GenericRepository<T> : IGenericRepository<T> where T : class
    {
        ...
        {
            if (!_cacheService(cacheTech).TryGet(cacheKey, out IReadOnlyList<T> cacheList))
            {
                cacheList = await _dbContext.Set<T>().ToListAsync();
                _cacheService(cacheTech).Set(cacheKey, cacheList);
            }
            return cacheList;
        }
